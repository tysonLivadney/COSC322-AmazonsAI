...
public class COSC322Test extends GamePlayer {
    ...
    private void makeAndSendMove() {
        ...
        // Validate BEFORE applying or sending
        if (!validateMove(myMove, boardBeforeMove)) {
            System.out.println("ERROR: AI returned an illegal move — aborting send.");
            return;
        }

        // Only apply and send if valid
     ...
     */
   private boolean validateMove(Move m, ArrayList<Integer> board) {
   ...
   private class AIPlayer {
    ...
    Move chooseMove(ArrayList<Integer> boardCopy, int color) {
        ...
        long timeLimit = startTime + 30000;
    ...
    private ArrayList<Integer> applyMoveToClone(ArrayList<Integer> board, Move m, int color) {
        ArrayList<Integer> next = new ArrayList<>(board);
        next.set(m.fr * 11 + m.fc, 0);
        next.set(m.tr * 11 + m.tc, color);
        next.set(m.ar * 11 + m.ac, 3);
        return next;
    }
    ...
    private int localFreedomAt(ArrayList<Integer> board, int r, int c) {
        ...
        for (int[] d : dirs) {
            ...
            if (nr >= 1 && nr <= 10 && nc >= 1 && nc <= 10) {
                if (board.get(nr * 11 + nc) == 0) freedom++;
            }
        }
        return freedom;
    }
    ...
    private ArrayList<ScoredMove> generateOrderedMoves(ArrayList<Integer> board, int color) {
        ...
        for (int[] queen : getQueenPositions(board, color)) {
            ...
            for (int[] newPos : getLegalMoves(board, fr, fc)) {
                ...
                // Temporarily move queen to generate arrow moves
                ArrayList<Integer> afterQueen = new ArrayList<>(board);
                afterQueen.set(fr * 11 + fc, 0);
                afterQueen.set(tr * 11 + tc, color);

                for (int[] arrow : getLegalMoves(afterQueen, tr, tc)) {
                    int ar = arrow[0], ac = arrow[1];
                    int score = quickMoveScore(afterQueen, color, tr, tc, ar, ac);
    ...
    private Move minimaxRoot(ArrayList<Integer> board, int depth, int color, long timeLimit) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            ArrayList<Integer> next = applyMoveToClone(board, m, color);

            int score = minimax(next, depth - 1, false, color, opponent,
    ...
    private int minimax(ArrayList<Integer> board, int depth, boolean isMaximizing,
        ...
        for (int i = 0; i < moveLimit; i++) {
            ScoredMove m = moves.get(i);
            ArrayList<Integer> next = applyMoveToClone(board, m, current);

            int score = minimax(next, depth - 1, !isMaximizing,
    ...
    // --- All evaluation methods below are unchanged ---

    private int countArrows(ArrayList<Integer> board) {
        int count = 0;
        for (int row = 1; row <= 10; row++)
            for (int col = 1; col <= 10; col++)
                if (board.get(row * 11 + col) == 3) count++;
        return count;
    }
    ...
    private int evaluate(ArrayList<Integer> board, int myColor) {
        ...
        if (arrows < 12) {
            return 3*mobilityScore + 8*territoryScore + 4*trappedScore + 3*localFreedomScore + 6*regionScore + 5*pressure;
        } else if (arrows < 35) {
            return 4*mobilityScore + 6*territoryScore + 8*trappedScore + 2*localFreedomScore + 5*regionScore + 7*pressure;
        } else {
            return 7*mobilityScore + 3*territoryScore + 12*trappedScore + 2*localFreedomScore + 6*regionScore + 9*pressure;
        }
    }
    ...
    private int countTrappedQueens(ArrayList<Integer> board, int color) {
        ...
        for (int[] queen : getQueenPositions(board, color)) {
            ...
            if (mobility <= 2) trapped += 2;
            else if (mobility <= 5) trapped += 1;
        }
        return trapped;
    }
    ...
    private int localFreedom(ArrayList<Integer> board, int color) {
        ...
        for (int[] queen : getQueenPositions(board, color)) {
            for (int[] d : dirs) {
                int nr = queen[0] + d[0], nc = queen[1] + d[1];
                if (nr >= 1 && nr <= 10 && nc >= 1 && nc <= 10)
                    if (board.get(nr * 11 + nc) == 0) freedom++;
            }
        }
    ...
    private int evaluateTerritory(ArrayList<Integer> board, int myColor, int opponent) {
        ...
        for (int row = 1; row <= 10; row++) {
            for (int col = 1; col <= 10; col++) {
                ...
                int md = myDist[row][col], od = oppDist[row][col];
